...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,me.cqp.siscon.CytusCalc";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("me.cqp.siscon.CytusCalc", 1, 9, "Cytus计算器", "3.0.0", 1, "siscon", "具体功能请私聊或在群组内发送（/cytus help或/cy help）", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("me.cqp.siscon.CytusCalc", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("me.cqp.siscon.CytusCalc", AppData.CQLog);	
